...
class Phew:
  ...
  async def _handle_request(self, reader, writer):
    ...
    # a handler that returns nothing would otherwise take the connection down
    # with it, leaving the socket open and the client waiting
    if response is None:
  ...
  def run_as_task(self, loop, host = "0.0.0.0", port = 80, ssl=None):
    loop.create_task(asyncio.start_server(self._handle_request, host, port, ssl=ssl))

  def run(self, host = "0.0.0.0", port = 80, ssl=None):
    logging.info("> starting web server on port {}".format(port))
    self.loop.create_task(asyncio.start_server(self._handle_request, host, port, ssl=ssl))
...
def add_route(path, handler, methods=None):
  default_phew().add_route(path, handler, methods)
